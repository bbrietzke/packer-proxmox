...
source "proxmox-iso" "rocky10" {
    ...
    vm_name                     = "rocky10-${formatdate("YYYYMMDD", timestamp())}"
    vm_id                       = "${var.machine_id}" 
    template_description        = "Rocky Linux 10"

    boot_iso {
        iso_file                = "${var.rocky_iso}"
        iso_storage_pool        = "${var.iso_storage_pool}"
        unmount                 = true
        iso_checksum            = "none"
    }
    
    template_name               = "rocky10-${formatdate("YYYYMMDD", timestamp())}"
