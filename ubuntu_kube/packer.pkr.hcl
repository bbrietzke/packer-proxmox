...
source "proxmox-iso" "ubuntu-server-kubernetes" {
    ...
    template_name               = "kubernetes-${formatdate("YYYYMMDD", timestamp())}"
...
build {
    ...
    sources = ["proxmox-iso.ubuntu-server-kubernetes"]
    ...
    provisioner "shell" {
        script = "ubuntu_kube/files/kubernetes-install.sh"
    }
